...
export default function AddCard(props: any) {
  ...
  return (
    <>
      <Modal title="Add Credit / Debit Card" open={props.show} onOk={props.clickOk} onCancel={props.clickCancel} centered footer={null}>
        <Form layout="vertical" onFinish={onFinish} onFinishFailed={onFinishFailed} autoComplete="off" initialValues={formValues}>
          <Form.Item label="Card Number" name={"usacAccountNumber"} rules={[{ required: true, message: "Please input card number!" }]}>
            <Input type="text" onChange={handleInputChange("usacAccountNumber")} placeholder="Enter Card Number" maxLength={16} size="large" className="focus:border-[#754cff] hover:border-[#754cff]" />
          </Form.Item>
          <Row gutter={16}>
            <Col>
              <Form.Item label="Bank" name={"usacEntityId"} rules={[{ required: true, message: "Please select your bank!" }]}>
                <Select
                  size="large"
                  allowClear
                  showSearch
            ...
            <Col>
              <Form.Item label="Type Card" name={"usacType"} rules={[{ required: true, message: "Please select type your card!" }]}>
                <Select size="large" allowClear placeholder="Select Card Type" style={{ width: 200 }} onChange={(value) => setFormValues({ ...formValues, usacType: value })}>
