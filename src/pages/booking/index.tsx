  ...
  const locationHotel = hotel.filter((item:any) => {
      if (!location) {
          return item;
            ...
            return item?.city.toLowerCase().includes((location as string).toLowerCase());
    }
  })
                              ...
                              <Col span={18}>
                                <Carousel autoplay autoplaySpeed={5000}>
                                  {arrPict?.map((each: any) => (
                                    <img className="w-full" src={`http://localhost:3600/facility-photos/${each}`} alt="hotels" key={each}/>
                                  ))}
                                </Carousel>
                              </Col>
                              <Col span={6}>
