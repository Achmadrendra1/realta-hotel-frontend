...
import { Button, Card, Carousel, Col, DatePicker, Divider, Form, Input, InputNumber, Modal, Pagination, Progress, Row, Select, Table, Tag } from "antd";
...
import { getBorde, getSpFacilities, getSpHotel, getSpInvoice, getSpReview, insertBookingExtra } from "@/Redux/Action/Booking/BookingAction";
...
import { doBankRequest, doPagaRequest, doUsacRequest } from "@/Redux/Action/Payment/paymentDashAction";
...
export default function bookingRoom() {
  ...
  const columnsExtra = [
    ...
    },
    {
      ...
      key: "action",
      render: (text: any, record: any, index: any) => <button onClick={() => handleDelete(index)}>Delete</button>,
    },
  ];
  ...
  const disabledDate = (current: any, checkInDate: any) => {
    if (checkInDate) {
      return current < dayjs().startOf("day") || current.isBefore(checkInDate, "day");
    }
  ...
  const bankAcc = userAcc?.filter((obj: any) => obj.usacType === "Credit Card" || obj.usacType === "Debet");
  const fintechAcc = userAcc?.filter((obj: any) => obj.usacType === "Payment");

  //Check Status Account H-Pay
  const accDompet = fintechAcc?.find((item: any) => item.usacAccountNumber == accNumberDompet);
  ...
  const accGoto = fintechAcc?.find((item: any) => item.usacAccountNumber == accNumberGoto);
  useEffect(() => {
  ...
  useEffect(() => {
    if (+selectCard.balance < dataBooking.boor_total_amount) {
      setPayMsg("Your Card Balance Is Insufficient, Please Check Or Select Another Card !");
      setDisaled(true);
    } else {
      setPayMsg("");
      setDisaled(false);
  ...
  return (
    <Layouts>
      {openAdd ? <AddCard show={openAdd} clickOk={handleOk} clickCancel={handleCancel} handleAct={handleActive} handleCancell={handleClose} dataUser={user} dataBank={allBank} /> : null}
      {showActivation ? <ActivationHpay show={showActivation} clickOk={handleOk} clickCancel={handleCancel} handleCancell={handleClose} phone={accNumberDompet} dataUser={user} dataPaga={payPaga} /> : null}
      {showLinked ? <ActivationGoto show={showLinked} clickOk={handleOk} clickCancel={handleCancel} handleCancell={handleClose} phone={accNumberGoto} dataUser={user} dataPaga={payPaga} /> : null}
      {showCheck ? <CheckSecure show={showCheck} clickOk={handleOk} clickCancel={handleCancel} handleCancell={handleClose} dataPayment={dataPayment} dataBooking={dataBooking} /> : null}
        ...
        <Row gutter={16}>
                  ...
                  return (
                    <div>
                          ...
                          <div>
                            <div className="flex border-0 rounded items-center justify-center w-20 h-10 text-2xl">
                              ...
                              <div className="flex justify-center">
                                <img className="ml-2 w-5 h-5" src="../../assets/star.png" alt="star" />
                              </div>
                            </div>
                            <div className="flex">
                              <div className="flex justify-center">
                      ...
                      <div className="text-l mt-2">
                        <p className="italic">{hotel.length > 0 ? <Link href={`${maps}${hotel[0].place}`}>{hotel[0].place}</Link> : hotel.place}</p>
                      </div>
                      ...
                      <div className="flex">
                        ...
                        <div className={`${more ? "block flex items-center mr-5" : "hidden"}`}>
                        ...
                        <div className="flex items-center">
                          <button onClick={() => setMore(!more)} className={`${more ? "hidden" : "block"}`}>
                            +View More
                          </button>
                          <button onClick={() => setMore(!more)} className={`${!more ? "hidden" : "block"}`}>
                            -Less More
                  ...
                  return (
                    <div>
                      <Card className="mb-3 shadow-lg">
                        <Row>
                          <Col span={16}>
                            <div className="text-xl font-semibold">{room.faci_name}</div>
                            <div>Max Vacant : {room.faci_max_number}</div>
                            <div className="flex">
                              <div className="flex text-xl items-center mr-3 text-red-500 font-semibold">{room.faci_rate_price}</div>
                              <div className="flex text-l text-decoration-line: line-through items-center">{room.faci_high_price}</div>
                            </div>
                          </Col>
                          <Col span={8}>
                            <div className="float-right">
                              <div className="flex justify-center">
                                <img src={`${configuration.BASE_URL}/facility-photos/${pict1[0]}`} alt="" className="w-3/4 rounded border-2" />
                              </div>
                              <div className="flex justify-center mt-5">
                                <Buttons funcs={() => handleButtonSelected(index)}>Select Room</Buttons>
                              </div>
                            </div>
                          </Col>
                        </Row>
                  ...
                  <Col span={10} className="flex justify-center">
                    <div>
                      <div className="flex border-0 rounded items-center justify-center w-20 h-10 text-2xl">
                        <h2>{rating}</h2>
                        <div className="flex justify-center">
                          <img className="ml-2 w-5 h-5" src="../../assets/star.png" />
                        </div>
                      </div>
                      ...
                      <div>
                        <div className="flex">
                          ...
                          <div className="items-center">
                            <img className="mx-1 w-5 h-5" src="../../assets/star.png" />
                          </div>
                          <div className="w-full">
                            <Progress percent={Math.round((rating5 / allReview) * 100)} format={() => <span style={{ color: "#000" }}>{Math.round((rating5 / allReview) * 100)}%</span>} strokeColor="" trailColor="" />
                          </div>
                        </div>
                      </div>
                      <div>
                        <div className="flex">
                          ...
                          <div className="w-full">
                            <Progress percent={Math.round((rating4 / allReview) * 100)} format={() => <span style={{ color: "#000" }}>{Math.round((rating4 / allReview) * 100)}%</span>} />
                          </div>
                        </div>
                      </div>
                      <div>
                        <div className="flex">
                          ...
                          <div className="w-full">
                            <Progress percent={Math.round((rating3 / allReview) * 100)} format={() => <span style={{ color: "#000" }}>{Math.round((rating3 / allReview) * 100)}%</span>} />
                          </div>
                        </div>
                      </div>
                      <div>
                        <div className="flex">
                          ...
                          <div className="w-full">
                            <Progress percent={Math.round((rating2 / allReview) * 100)} format={() => <span style={{ color: "#000" }}>{Math.round((rating2 / allReview) * 100)}%</span>} />
                          </div>
                        </div>
                      </div>
                      <div>
                        <div className="flex">
                          ...
                          <div className="w-full">
                            <Progress percent={Math.round((rating1 / allReview) * 100)} format={() => <span style={{ color: "#000" }}>{Math.round((rating1 / allReview) * 100)}%</span>} />
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              {oneReview &&
                oneReview.map((hotelReview: any) => {
                  return (
                    <div className="mb-2">
                      <div className="flex items-center">
                        <div className="text-xl mr-5 font-semibold">{hotelReview.user_full_name}</div>
          ...
          <Col span={14} className={`${detail ? "block" : "hidden"}`}>
            ...
            >
              <div className="flex my-2">
                ...
                <div className="flex font-semibold text-xl items-center">Modify your booking</div>
              </div>
            </button>
            ...
            <div className="text-xl items-center my-3">
              <Modal title="Add Extra" centered open={addExtra} onOk={() => setAddExtra(false)} onCancel={() => setAddExtra(false)} footer={null} width={750}>
                <div>
              ...
              <div className="w-full h-screen p-8">
                ...
                <Row gutter={16}>
                  <Col span={8}>
                    <Card
                      size="small"
                      className={`m-4 font-bold text-[14px] ${isCash ? "bg-[#754cff] text-white" : ""}`}
                      hoverable
                      onClick={() => {
                        setIsCash(true),
                          setDataPayment({
                  ...
                  <Col span={16}>
                    {isCash ? (
                      <div className="w-full text-center mt-6">
                        <div>
                          <p className="text-lg font-semibold">No payment is needed at the moment.</p>
                          <p>We will confirm your stay without any charge. Pay directly at the hotel during your stay.</p>
                        </div>
                        <Button onClick={onCompleteCash} className="mt-6 bg-[#754cff] text-white w-full h-10">
                          Complete Order
                        </Button>
                      </div>
                    ) : (
                      <div className="mt-6 w-full ">
                        ...
                        {isActive ? (
                          <Card
                            size={"small"}
                            className={`mb-2 ${selectCard.accNumber === accDompet?.usacAccountNumber && "bg-[#754cff] text-white"}`}
                            hoverable
                            onClick={() => {
                              setSelectCard({
                          ...
                          >
                            <div className="flex justify-between items-center px-6">
                              <p className="text-[16px] font-semibold">H-Pay</p>
                              <p className="text-[16px] font-semibold">{saldoDompet}</p>
                            </div>
                          </Card>
                        ) : (
                          <Card size={"small"} className="mb-2">
                            <div className="flex justify-between items-center px-6">
                              <p className="text-[16px] font-semibold">H-Pay</p>
                              <p className={`${selectCard.accNumber === accDompet?.usacAccountNumber ? "text-white" : "text-blue-700"} hover:cursor-pointer`} onClick={() => setShowActivation(true)}>
                                Activate
                              </p>
                            </div>
                          </Card>
                        ...
                        {isLinked ? (
                          <Card
                            size={"small"}
                            className={`mb-2 ${selectCard.accNumber === accGoto?.usacAccountNumber && "bg-[#754cff] text-white"}`}
                            hoverable
                            onClick={() => {
                              setSelectCard({
                          ...
                          >
                            <div className="flex justify-between px-6">
                              ...
                              <p className="text-[16px] font-semibold">{saldoGoto}</p>
                            </div>
                          </Card>
                        ) : (
                          <Card size={"small"} className="mb-2">
                            <div className="flex justify-between px-6">
                              <p className="text-[16px] font-semibold">GOTO</p>
                              <p className={`${selectCard.accNumber === accGoto?.usacAccountNumber ? "text-white" : "text-blue-700"} hover:cursor-pointer`} onClick={() => setShowLinked(true)}>
                                Link Account
                              </p>
                            </div>
                          </Card>
                        ...
                        {bankAcc.map((item: any) => (
                          <Card
                            size={"small"}
                            className={`mb-2 ${selectCard.accNumber === item.usacAccountNumber && "bg-[#754cff] text-white"}`}
                            hoverable
                            onClick={() => {
                              setSelectCard({
                          ...
                          >
                            <div className="flex justify-between px-6">
                              <p className="text-[16px] font-semibold">{maskCardNumber(item.usacAccountNumber)}</p>
                              <p>
                                {allBank?.find((obj: any) => obj.bankEntityId == item.usacEntityId)?.bankName} - {item.usacType}
                              </p>
                            </div>
                          </Card>
                        ))}

                        <p className="mt-2 px-2 cursor-pointer" onClick={() => setOpenAdd(true)}>
                          Add New Card
                        </p>
                        <Button disabled={disabled} onClick={onComplete} className="mt-6 bg-[#754cff] text-white w-full h-12">
                          Complete Order
                        </Button>
                      </div>
                    )}
          ...
          <Col span={10}>
            <div className="sticky top-0 border-4 shadow-lg p-5 rounded-lg bg-white">
              <div className="flex justify-center font-bold text-3xl">Booking Order Details</div>
              <h1 className="font-bold text-lg mt-4">{dataBooking.boor_order_number}</h1>
              ...
              <Row gutter={10}>
                <Col span={12}>
                  <RangePicker onChange={handleDateRangeChange} format={dateFormat} disabledDate={(current) => disabledDate(current, dayjs())} />
                </Col>
              ...
              <div className="flex justify-between items-center mt-2 mb-3">
                <div className=" flex my-1 items-center">
                  <Modal title="Special Offers" centered open={spofOpen} onOk={() => setSpofOpen(false)} onCancel={() => setSpofOpen(false)} footer={null} width={750}>
                    {typeSpof &&
                ...
                <div className="text-xl font-semibold italic">{spofPrice.spofName}</div>
                <div>
                  <button onClick={handleDeleteSpof} className={`${showTrash ? "block" : "hidden"}`}>
                    <DeleteOutlined className="text-red-500" />
                  </button>
                </div>
                <div className="flex text-xl my-1 items-center">- {spofPrice.spofDiscount}</div>
              </div>
              ...
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex text-xl items-center">- {spofPrice.spofDiscount}</div>
              </div>
              ...
              <div className="flex justify-center mt-3">
                <Button onClick={handleReservation} className={`text-white bg-[#754CFF] ${detail ? "hidden" : "block"}`}>
                  Reservation Booking
                </Button>
                <Button onClick={handleBookingCode} className={`text-white bg-[#754CFF] ${!detail || payment ? "hidden" : "block"}`}>
