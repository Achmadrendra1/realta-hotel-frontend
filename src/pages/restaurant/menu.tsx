...
import { Affix, Breadcrumb, Button, Modal, Pagination, Radio, Select, Tag } from "antd";
...
export default function menu({ restaurant }: any) {
  ...
  const addtocart = (menu: any) => {
    if (cart.length === 0 || menu.remefaciid == cart[0]?.remefaciid) {
      ...
      if (!itemInCart) {
        ...
        let numberOfPrice = Number(menu.remeprice.split(",")[0].replace(/[^0-9]/g, ""));
  ...
  const setQuantity = (product: any, amount: any) => {
    ...
    newCart.find((item: any) => item.remename === product.remename).quantity = amount;
    setCart(newCart);
  };
    ...
    <>
      ...
      <main>
        <Layouts>
          <div className="container mx-auto">
            ...
            <div className="mt-3 lg:flex  my-20">
              <div className="lg:w-3/5 sm:full sm:mb-4 border rounded-xl shadow p-3 lg:mr-2 bg-white lg:mb-0">
                <div className="text-xl font-bold text-center">Menus</div>
                ...
                <div className="flex flex-wrap ml-5 item-center justify-center">
                  {listmenu &&
                    listmenu.map((menu: any, key: any) => (
                      <div key={menu.remeid} className="w-52 mr-6 mb-12">
                        <div>
                          <a onClick={() => showModalsMenu(menu)} className=" hover:text-[#754cff]">
                            <img src={`${configuration.BASE_URL}/resto-menu-photos/menu/${menu.rempphotofilename}`} alt={menu.remename} className="h-40 w-full object-cover rounded-lg"></img>
                            <div className="ml-3 mt-3 h-40">
                              <p className="text-lg font-bold h-20">{menu.remename}</p>
                          ...
                          <div className="w-full flex mt-2">
                            {userid ? (
                              <button onClick={() => addtocart(menu)} className="w-3/4 mx-auto rounded-full inline-block px-5 py-2 mt-4 bg-[#754cff] hover:bg-[#592fe4] text-white uppercase bottom-0">
              ...
              {getCartTotal() === 0 || !userid ? (
                  ...
                  <div className="border font-semibold bg-white rounded-lg shadow py-20 lg:ml-2 text-xl text-center">
                    ...
                    {!userid ? (
                      <div>
                        ...
                        <br />
                        <br />
                        <Buttons funcs={login}>Login</Buttons>
                      </div>
                    ) : (
                ...
                <div className="bg-white rounded-lg shadow p-3 sticky top-0 h-1/2 lg:w-2/5 sm:w-full">
                  ...
                  <div className="text-lg pt-5 font-semibold">Total Order : {getCartTotal()}</div>
                  {cart &&
                    cart.map((order: any) => (
                      <div className="border rounded-lg p-4 shadow-md gap-y-2 my-5 flex" key={order.remeid}>
                        <img src={`${configuration.BASE_URL}/resto-menu-photos/menu/${order.rempphotofilename}`} alt={order.remename} width={120} height={120} className="object-cover"></img>
                        <div className="ml-3 mt-1 w-full">
                          <div className="flex justify-between">
                            <p className="font-bold w-4/5">{order.remename}</p>
                            <button onClick={() => removeFromCart(order)} className="text-red-600 text-md mr-4">
                              <CloseOutlined />
                            </button>
                          </div>
                          {/* <p>{order.desc}</p> */}
                          <p>
                            {parseInt(order.remeprice.split(",")[0].replace(/[^0-9]/g, "")).toLocaleString("id-ID", {
                              style: "currency",
                              currency: "IDR",
                          ...
                          <div>
                            {/* // quantity by incdecr */}
                            <button onClick={() => dec(order)} className="text-xl text-indigo-700">
                              <MinusCircleOutlined className=" hover:bg-indigo-100 hover:rounded-full" />
                            </button>
                            {"   " + order.quantity + "   "}
                            <button onClick={() => inc(order)} className="text-xl text-indigo-700">
                              <PlusCircleOutlined className=" hover:bg-indigo-100 hover:rounded-full" />
                            </button>
                          </div>
                          <p>
                            Subtotal:{" "}
                  ...
                  {/* <Link href='order'> */}
                  <a>
                    <div onClick={placeOrder} className="border rounded-full bg-[#754cff] my-5 text-center font-bold py-4 text-[#f2f1fa] hover:text-white">
                      PLACE ORDER HERE
                    </div>
                  </a>
                  {/* </Link> */}
          ...
          >
            <div className="text-center text-xl font-bold">Items Previously Selected</div>

            {cart &&
              cart.map((order: any) => (
                <div className="border rounded-lg p-4 shadow-md gap-y-2 my-5 flex" key={order.remeid}>
                  <img src={`${configuration.BASE_URL}/resto-menu-photos/menu/${order.rempphotofilename}`} alt={order.remename} width={120} height={120} className="object-cover" />
                  <div className="ml-3 mt-1 w-full">
                    <div className="flex justify-between">
                      <p className="font-bold w-4/5">{order.remename}</p>
                    </div>
