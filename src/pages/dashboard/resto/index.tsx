...
export default withAuth(function restoMenu() {
  ...
  function saveNewPhoto(e: any) {
    ...
    axios
      .post("http://localhost:3501/resto-menu-photos", formData, {
        headers: {
      ...
      {role == "Manager" || role == "Admin" ? (
        <Dashboard>
          ...
          <div className="my-4 px-4">
            <div className="float-right right-0 items-right"></div>
          ...
          <Table dataSource={menus.data} pagination={{ current: currentpage, onChange: handlePagination, total: menus.counts }} className="py-4">
            ...
            <Table.Column
              title="Price"
              dataIndex="reme_price"
              key="reme_price"
              render={(rp) => {
                return rp.replace("$", "Rp.");
              }}
            />
            ...
            <Table.Column
              title="Modified Date"
              dataIndex="reme_modified_date"
              key="reme_modified_date"
              render={(dateString) => {
            ...
            <Table.Column
              title={
                <button onClick={showAddMenu} className="bg-slate-200 hover:bg-slate-300 rounded-lg w-24 py-3">
              ...
              render={(_: any, record: any) => (
                <Space size="middle">
                  <Dropdown
                    overlay={
                      <Menu>
                        <Menu.Item key={"edit"} onClick={() => editMenu(record.reme_id)}>
                          {" "}
                          <EditOutlined className="mr-2" />
                          Edit Menu
                        </Menu.Item>
                        {/* <Menu.Item key={'add'} onClick={() => photoMenu(record)}><UploadOutlined className='mr-2'/>Upload photo</Menu.Item> */}
                        <Menu.Item key={"addmultiple"} onClick={() => addMultiple(record)}>
                          <UploadOutlined className="mr-2" />
                          Upload Photo
                        </Menu.Item>
                        <Menu.Item key={"update"} onClick={() => thumbnailMenu(record)}>
          ...
          >
            <Form
              ...
              className="mx-auto"
            >
              <p className="text-center text-xl py-5 font-bold">Update Menu Restaurant</p>
              <>
                <Form.Item name="remeFaciId" label="Facility" rules={[{ required: true, message: "Please select restaurant!!" }]}>
                  <Select value={firstValue} onChange={(e) => handleSelection(e, "remeFaci")}>
                    <>
                      {list_restaurant &&
                        list_restaurant.map((resto: any) => (
                          <Select.Option value={resto.faci_id}>
                            {resto.hotel_name} - {resto.faci_name}
                          </Select.Option>
                        ))}
                    </>
                  </Select>
                ...
                <Form.Item name={"remeName"} label="Name Menu" rules={[{ required: true, message: "Please input name menu!" }]}>
                  <Input type="text" onChange={eventHandler("remeName")} value={getMenu.remeName} />
                  <p className="pl-3 text-black" hidden>
                    Previous: {getMenu.remeName}
                  </p>
                ...
                <Form.Item name="remeDescription" label="Description" rules={[{ required: true, message: "Please input description!" }]}>
                  ...
                  <p className="pl-3 text-white" hidden>
                    Previous: {getMenu.remeDescription}
                  </p>
                </Form.Item>

                <Form.Item name="remePrice" label="Price" rules={[{ required: true, message: "Please input price!" }]}>
                  <Input onChange={eventHandler("remePrice")} value={getMenu.remePrice.toString()} />
                  <p className="pl-3 text-white" hidden>
                    Previous: {getMenu.remePrice}
                  </p>
                </Form.Item>

                <Form.Item name="remeStatus" label="Status" rules={[{ required: true, message: "Please select status!" }]}>
                  <Switch onChange={switchMenu} className="bg-slate-400" defaultChecked />
                  <p className="pl-3 text-white" hidden>
                    Previous: {getMenu.remeStatus}
                  </p>
                </Form.Item>
              </>
            </Form>
          ...
          <Modal
            ...
            footer={[
              <>
                <Button key="back" onClick={handleCancelAddMenu}>
                  Cancel
                </Button>
                {contextHolder}
                <Button
          ...
          >
            <Form form={form} onFinish={onFinish} labelCol={{ span: 8 }} wrapperCol={{ span: 14 }} layout="horizontal" style={{ maxWidth: 900 }} className="mx-auto">
              <p className="text-center text-xl py-5 font-bold">Add Menu Restaurant</p>
              <>
                <Form.Item name="remeFaciId" label="Restaurant" rules={[{ required: true, message: "Please select restaurant!!" }]}>
                  <Select placeholder={"Select restaurant"} onChange={(e) => handleSelection(e, "remeFaci")}>
                    {list_restaurant &&
                      list_restaurant.map((resto: any) => (
                        <Select.Option value={resto.faci_id} key={resto.faci_id}>
                          {resto.faci_name} - {resto.hotel_name}
                        </Select.Option>
                      ))}
                  </Select>
                </Form.Item>
            ...
            >
              ...
              <Form.Item name="rempUrl" label={""}>
                <div className="h-96 w-64 border text-center">
                  ...
                  <br></br>
                  <input className="file:w-24 file:border file:border-dashed file:border-slate-500 file:bg-white " type="file" id="primary" accept="image/*" onChange={handleUploadChange}></input>
                  <br />
                  <Buttons funcs={saveNewPhoto}>Save Photo</Buttons>
                </div>
              </Form.Item>
                        ...
                        return (
                          <>
                            ...
                            <Form.Item name={[field.name, "rempUrl"]} label={"first photo"}>
                              ...
                              <input type="file" id={index.toString()} accept="image/*" onChange={handleUploadChange}></input>
          ...
          <Modal
            ...
            footer={[
              <>
                <Button key="back" onClick={handleCancelThumbnail}>
                  Cancel
                </Button>
            ...
            {viewThumbnailPhoto.length > 0 ? (
              <>
                ...
                <div className="mb-4">
                  Guides:
                  <ul>
                    <li>- Select photo to set new thumbnail photo</li>
                    <li>- Klik delete to delete permanent photo</li>
                  </ul>
                </div>
          ...
          <Modal
            ...
            footer={[
              <>
                <Button key="back" onClick={cancelAddMultiple}>
                  Cancel
                </Button>
                {contextHolder}
                <Button
                  key="submit"
                  onClick={saveMultiplePhoto}
                  className="bg-sky-500 text-white"
                  onMouseEnter={(e: any) => {
                    e.target.style.color = "#fff";
                  }}
          ...
          >
            ...
            >
              ...
              <Form.Item label="Upload photo">
                ...
                <input type="file" id="rempUrl" name="rempUrl" accept="image/*" onChange={handleInputMultiple} multiple className="file:w-24 file:border file:border-dashed file:border-slate-500 file:bg-white " />
